  ...
  return (
    <div className={`popup popup_type_${ props.name } popup_transition ${ props.isOpen ? 'popup_opened' : '' }`} onMouseDown={props.onClose}>
      <div className="popup__form-container popup__form-container_type_infoTooltip" onMouseDown={ (e)=>{e.stopPropagation()} }>
          <img src={props.isOk ? granted : denied} className="popup__logo" alt={props.isOk ? `Вы успешно зарегистрировались!` : `Что-то пошло не так! Попробуйте ещё раз.` } />
          <h2 className="popup__title popup__title_type_infoTooltip">{ props.isOk ? `Вы успешно зарегистрировались!` : `Что-то пошло не так! Попробуйте ещё раз.` }</h2>
