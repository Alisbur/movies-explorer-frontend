...
class MainApi {
  constructor({ server, registerPath, loginPath, authCheckPath, profileDataPath, moviesDataPath }) {
    this._server = server;
  ...
  addSavedMovie({ country, director, duration, year, description, image, trailerLink, thumbnail, movieId, nameRU, nameEN }, token) {
    const path = this._server + this._moviesDataPath;
    const message = { 
      method: 'POST',
      headers: {
